...
Tour::Tour(int n) : cached_cost(-1.0), cached_value(std::numeric_limits<double>::lowest()) {
    vertices.resize(n);
    vertices[0] = 1;  
    for (int i = 1; i < n; ++i) {
        vertices[i] = i + 1;
...
Tour::Tour(const Tour& other) : vertices(other.vertices), 
                                cached_cost(other.cached_cost),
...
double Tour::compute_value(const InputData& input, const std::vector<int>& new1_to_old0) const {
    if (cached_value > std::numeric_limits<double>::lowest()) {
        return cached_value;
    }
...
double Tour::compute_cost(const InputData& input, const std::vector<int>& new1_to_old0) const {
    if (cached_cost >= 0.0) return cached_cost;
    ...
    for (size_t i = 1; i < vertices.size(); ++i) {
        ...
        if (to_old != 0) { // 0 - это депо в старых индексах
            current_time += input.point_service_times[to_old-1];
        }
    }
...
void Tour::invalidate_cache() {
    cached_cost = -1.0;
    cached_value = std::numeric_limits<double>::lowest();
}